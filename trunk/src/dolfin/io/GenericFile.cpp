...
void GenericFile::write()
{
  ...
  if (!opened_write)
  {
    ...
    if (!file.good())
    {
      printf("GenericFile.cpp\n                   write to file\n                   Unable to open file \"%s\" for writing", filename.c_str());
    }
    file.close();
  }
  opened_write = true;
...
{
  printf("GenericFile.cpp\n               read object from file\n               Cannot read objects of type %s from %s files", object.c_str(), filetype.c_str());
}
//-----------------------------------------------------------------------------
void GenericFile::write_not_impl(const std::string object) const
{
  printf("GenericFile.cpp\n               write object to file\n               Cannot write objects of type %s to %s files",object.c_str(), filetype.c_str());
}
//-----------------------------------------------------------------------------
